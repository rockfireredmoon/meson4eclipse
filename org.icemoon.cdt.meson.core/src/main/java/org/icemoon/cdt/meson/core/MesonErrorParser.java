 ...
 */
public class MesonErrorParser extends AbstractErrorParser implements
    IErrorParser2, IErrorParser3 {

  public static final String MESON_PROBLEM_MARKER_ID = CdtPlugin.PLUGIN_ID
      + ".problem"; //$NON-NLS-1$
   ...
   * <pre>
   * blah Specify
   * --help for usage, or press the help button on the Meson GUI.
   * </pre>
   */
  private static final String pattern_invoke_ignore = "Specify --help for usage, .+";
  /**
   * meson invocation error, with exit status 1:
   *
   * <pre>
   * Meson Error: The source directory "/home/gipsnich/build" does not exist.
   */
  private static final Pattern PTN_INVOKE_ERROR = Pattern
      .compile("^Meson Error:\\s*(.+)$");
  /**
   * Mesonlists.txt errors...
   *
   * <pre>
   * Meson Error at MesonLists.txt:14 (XXXproject):
   * </pre>
   */
  private static final Pattern PTN_SCRIPT_ERROR = Pattern
      .compile("^Meson Error at (.+):\\s*(\\d+)\\s+\\((.+?)\\):$");
  /**
   * Mesonlists.txt warnings...
   *
   * <pre>
   * Meson Warning at MesonLists.txt:14 (XXXproject):
   * </pre>
   */
  private static final Pattern PTN_SCRIPT_WARN = Pattern
      .compile("^Meson Warning at (.+):\\s*(\\d+)\\s+\\((.+?)\\):$");
  /**
   * Mesonlists.txt developer warnings...
   *
   * <pre>
   * Meson Warning (dev) in MesonLists.txt:
   * </pre>
   *
   ...
   *
   *     /home/weber/devel/src/CDT-Meson/CDT-mgmt-C-0src/MesonLists.txt:51 (IF)
   *
   *   closes on the line
   *
   *     /home/weber/devel/src/CDT-Meson/CDT-mgmt-C-0src/MesonLists.txt:54 (ENDIF)
   *
  ...
  private static final Pattern PTN_SCRIPT_WARN_DEV = Pattern
      .compile("^Meson Warning \\(dev\\) in (.+):\\s*(\\d+)\\s+.+?:$");
  /**
...
//    System.err.println("# " + line);
    ...
    case ERR_SCRIPT:
      ...
      if (line.startsWith("-- "))
        return true; // CDT or meson sometimes reorder lines, ignore
...
//      return true; // consume line
  ...
  private void createMarker(String fileName, ErrorParserManager epm,
    ...
    // meson reports the file relative to source entry
  ...
  private enum PState {
    ...
    ERR_SCRIPT,
    /** error in MesonLists.txt, first part of description seen */
    ERR_SCRIPT_DESCR_1,
    /**
     * error in MesonLists.txt, full description seen, wanting first empty line.
     * (Two empty lines mark the end of a Meson error message)
     */
    ERR_SCRIPT_END_1,
    /**
     * error in MesonLists.txt, full description seen, wanting second empty
     * line.
     */
    ERR_SCRIPT_END_2;
  }
