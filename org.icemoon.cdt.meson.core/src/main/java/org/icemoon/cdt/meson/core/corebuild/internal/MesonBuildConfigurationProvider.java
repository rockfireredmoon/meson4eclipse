...
public class MesonBuildConfigurationProvider implements ICBuildConfigurationProvider {
	...
	public ICBuildConfiguration createBuildConfiguration(IProject project, IToolChain toolChain, String launchMode,
		...
		for (IBuildConfiguration config : project.getBuildConfigs()) {
			...
			if (cconfig != null) {
				CBuildConfiguration cmakeConfig = cconfig.getAdapter(MesonBuildConfiguration.class);
		...
		// create config
		String configName = "meson." + launchMode + '.' + toolChain.getId(); //$NON-NLS-1$
		IBuildConfiguration config = configManager.createBuildConfiguration(this, project, configName, monitor);
		CBuildConfiguration autotoolsConfig = new MesonBuildConfiguration(config, configName);
