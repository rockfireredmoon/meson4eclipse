...
public abstract class AbstractOsPropertyTab<P extends AbstractOsPreferences> extends QuirklessAbstractCPropertyTab {
	...
	protected void createControls(Composite parent) {
		...
		{
			GridLayout layout;
			Group gr = WidgetHelper.createGroup(usercomp, SWT.FILL, 2, "Meson Executable", 2);

			b_cmdFromPath = WidgetHelper.createCheckbox(gr, SWT.BEGINNING, 2,
					"Use meson executable found on &system path");
			...
			b_cmdBrowseFiles.addSelectionListener(new SelectionAdapter() {
				...
				public void widgetSelected(SelectionEvent e) {
					...
					dialog.setFilterPath(settings.get("meson_dir"));
					String text = dialog.open();
					settings.put("meson_dir", dialog.getFilterPath());
					if (text != null) {
						t_cmd.setText(text);
					}
				}
		...
		{
			setupLabel(usercomp, "Backend (--backend):", 1, SWT.BEGINNING);
		...
		// meson defines table...
		final ICResourceDescription resDesc = getResDesc();
		projectOptionsViewer = new ProjectOptionsViewer(usercomp, resDesc == null ? null : resDesc.getConfiguration());
	}
	...
	private void updateDisplay() {
		t_cmd.setText(prefs.getCommand() == null ? "" : prefs.getCommand());
