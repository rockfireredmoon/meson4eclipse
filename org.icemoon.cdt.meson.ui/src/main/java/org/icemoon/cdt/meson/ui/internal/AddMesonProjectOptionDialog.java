...
/**
 * The dialog used to create or edit a meson define.
 *
 * @author Martin Weber
 * @author Emerald Icemoon
 */
public class AddMesonProjectOptionDialog extends Dialog {
	private static final String DIALOG_SETTINGS_SECT = "dlg_addMesonProjectOption"; //$NON-NLS-1$
	/** String representation of each {@code MesonProjectOptionType}, by ordinal */
	private static final String[] typeNames;

	static {
		MesonProjectOptionType[] variableTypes = MesonProjectOptionType.values();
	...
	/**
	 * Converts an index in {@link AddMesonProjectOptionDialog#typeNames} to a meson
	 * project option type.
	 *
	 * @param selectionIndex
	 */
	private static MesonProjectOptionType indexToType(int selectionIndex) {
		return MesonProjectOptionType.values()[selectionIndex];
	}
	 ...
	 * created.
	 */
	private ProjectOption editedVar;
	 ...
	 *            or <code>null</code>
	 */
	public AddMesonProjectOptionDialog(Shell parentShell, ICConfigurationDescription cfgd, ProjectOption editedVar) {
		super(parentShell);
	...
	/**
	 * Gets the edited or newly created meson project option.
	 *
	 * @return the modified or new ProjectOption or {@code null} if this dialog has
	 *         been cancelled.
	 */
	...
	protected void configureShell(Shell shell) {
		...
		if (editedVar != null)
			shell.setText(Messages.AddMesonProjectOptionDialog_1);
		else
			shell.setText(Messages.AddMesonProjectOptionDialog_2);
	}
	...
	protected Control createDialogArea(Composite parent) {
		...
		nameLabel.setText(Messages.AddMesonProjectOptionDialog_3);
		...
		variableName.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false, 2, 1));

		Label typeLabel = new Label(comp, SWT.NONE);
		typeLabel.setText(Messages.AddMesonProjectOptionDialog_4);
		...
		lblvalue.setText(Messages.AddMesonProjectOptionDialog_5);

		variableValue = new Text(comp, SWT.BORDER);
		GridData gd_variableValue = new GridData(SWT.FILL, SWT.CENTER, true, false, 2, 1);
		...
		btnBrowseFiles.setText(Messages.AddMesonProjectOptionDialog_6);
		btnBrowseFiles.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false, false, 1, 1));
		btnBrowseFiles.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (MesonProjectOptionType.PATH == indexToType(sel)) {
					DirectoryDialog dialog = new DirectoryDialog(getShell());
					dialog.setFilterPath(settings.get("dir")); //$NON-NLS-1$
					text = dialog.open();
					settings.put("dir", dialog.getFilterPath()); //$NON-NLS-1$
				} else {
					FileDialog dialog = new FileDialog(getShell());
					dialog.setFilterPath(settings.get("file")); //$NON-NLS-1$
					text = dialog.open();
					settings.put("file", dialog.getFilterPath()); //$NON-NLS-1$
				}
		...
		btnBrowseVars.setText(Messages.AddMesonProjectOptionDialog_11);
		btnBrowseVars.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false, false, 1, 1));
		if (cfgd == null) {
	...
	protected void okPressed() {
		if (editedVar != null) {
			...
			editedVar = new ProjectOption(variableName.getText(), indexToType(typeSelector.getSelectionIndex()),
	...
	/**
	 * Sets button enabledment based on meson variable type.
	 */
	private void adjustBrowseButtons(MesonProjectOptionType type) {
		switch (type) {
		case BOOL:
