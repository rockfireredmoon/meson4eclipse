...
public class NinjaBuildConfigurationProvider implements ICBuildConfigurationProvider {
	...
	public static final String ID = NinjaPlugin.getId() + ".provider"; //$NON-NLS-1$

	private ICBuildConfigurationManager configManager = NinjaPlugin.getService(ICBuildConfigurationManager.class);
	...
	public ICBuildConfiguration getCBuildConfiguration(IBuildConfiguration config, String name) throws CoreException {
		...
		if (toolChain != null) {
			return new NinjaBuildConfiguration(config, name, toolChain);
		} else {
			return null;
		}
	}
