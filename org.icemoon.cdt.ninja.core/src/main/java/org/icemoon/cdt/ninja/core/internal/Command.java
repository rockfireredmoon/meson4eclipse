...
/**
 * Makefile : ( statement ) * statement : command | .. command : <tab>
...
public class Command extends Directive implements ICommand {
	...
	/**
	 * - If the command prefix contains a hyphen, or the -i option is present, or
	 * the special target .IGNORE has either the current target as a prerequisite or
	 * has no prerequisites, any error found while executing the command will be
	 * ignored.
	 */
	@Override
	...
	/**
	 * '@' If the command prefix contains an at sign and the command-line -n option
	 * is not specified, or the -s option is present, or the special target .SILENT
	 * has either the current target as a prerequisite or has no prerequisites, the
	 * command will not be written to standard output before it is executed.
	 */
	@Override
	...
	/**
	 * + If the command prefix contains a plus sign, this indicates a command line
	 * that will be executed even if -n, -q or -t is specified.
	 */
	@Override
	...
	/**
	 * command : <tab> prefix_command string
	 * <nl>
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.eclipse.cdt.make.core.makefile.ICommand#execute(java.lang.String[],
	 * java.io.File)
	 */
	@Override
