...
public abstract class AbstractMakefileCodeScanner extends RuleBasedScanner {
	...
	/**
	 * Preference keys for boolean preferences which are <code>true</code>, iff the
	 * corresponding token should be rendered bold.
	 */
	private String[] fPropertyNamesBold;
	/**
	 * Preference keys for boolean preferences which are <code>true</code>, iff the
	 * corresponding token should be rendered italic.
	 */
	private String[] fPropertyNamesItalic;

	/**
	 * Returns the list of preference keys which define the tokens used in the rules
	 * of this scanner.
	 */
	...
	public final void initialize() {
		...
		for (int i = 0; i < length; i++) {
			fPropertyNamesBold[i] = fPropertyNamesColor[i] + NinjaFileEditorPreferenceConstants.EDITOR_BOLD_SUFFIX;
			fPropertyNamesItalic[i] = fPropertyNamesColor[i] + NinjaFileEditorPreferenceConstants.EDITOR_ITALIC_SUFFIX;
	...
	protected TextAttribute createTextAttribute(String colorID, String boldKey, String italicKey) {
		Color color = null;
		if (colorID != null) {
			color = NinjaUIPlugin.getPreferenceColor(colorID);
		}
