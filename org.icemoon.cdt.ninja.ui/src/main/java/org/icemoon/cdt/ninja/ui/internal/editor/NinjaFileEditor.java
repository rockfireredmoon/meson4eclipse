...
public class NinjaFileEditor extends TextEditor implements ISelectionChangedListener, IReconcilingParticipant {
	/**
	 * The page that shows the outline.
	 */
	protected NinjaContentOutlinePage page;
	...
	NinjaSourceConfiguration getMakefileSourceConfiguration() {
		SourceViewerConfiguration configuration = getSourceViewerConfiguration();
		if (configuration instanceof NinjaSourceConfiguration) {
			return (NinjaSourceConfiguration) configuration;
		}
		return null;
	}

	public NinjaContentOutlinePage getOutlinePage() {
		if (page == null) {
			page = new NinjaContentOutlinePage(this);
	...
	protected void initializeEditor() {
		...
		setDocumentProvider(NinjaUIPlugin.getDefault().getNinjaDocumentProvider());
		IPreferenceStore chainedStore = NinjaUIPlugin.getDefault().getCombinedPreferenceStore();
		setPreferenceStore(chainedStore);
		setSourceViewerConfiguration(new NinjaSourceConfiguration(chainedStore, this));
	}
	...
	boolean isFoldingEnabled() {
		return NinjaUIPlugin.getDefault().getPreferenceStore()
				.getBoolean(NinjaFileEditorPreferenceConstants.EDITOR_FOLDING_ENABLED);
	}

	@Override
	protected void rulerContextMenuAboutToShow(IMenuManager menu) {
		super.rulerContextMenuAboutToShow(menu);
		IMenuManager foldingMenu = new MenuManager(NinjaFileEditorMessages.MakefileEditor_menu_folding, "projection"); //$NON-NLS-1$
	...
	protected void createActions() {
		...
		a = new NinjaToggleCommentAction(bundle, "ToggleComment.", this); //$NON-NLS-1$
		a.setActionDefinitionId(INinjaEditorActionDefinitionIds.TOGGLE_COMMENT);
		setAction("ToggleComment", a); //$NON-NLS-1$
		markAsStateDependentAction("ToggleComment", true); //$NON-NLS-1$

		ISourceViewer sourceViewer = getSourceViewer();
		SourceViewerConfiguration configuration = getSourceViewerConfiguration();
		((NinjaToggleCommentAction) a).configure(sourceViewer, configuration);

		a = new OpenDeclarationAction(this);
		a.setActionDefinitionId(INinjaEditorActionDefinitionIds.OPEN_DECLARATION);
	...
	public void selectionChanged(SelectionChangedEvent event) {
		...
		if (selection.isEmpty()) {
			...
			if (!isActivePart() && NinjaUIPlugin.getActivePage() != null) {
				NinjaUIPlugin.getActivePage().bringToTop(this);
			}
	...
	public void setSelection(IDirective directive, boolean moveCursor) {
		...
		try {
			...
			if (moveCursor) {
				...
				IWordDetector detector = new NinjaWordDetector();
	...
	/**
	 * Removes the given listener. Has no effect if an identical listener was not
	 * already registered.
	 *
	 * @param listener
	...
	protected void handlePreferenceStoreChanged(PropertyChangeEvent event) {
		...
		NinjaSourceConfiguration makeConf = getMakefileSourceConfiguration();
		if (makeConf != null) {
		...
		if (NinjaFileEditorPreferenceConstants.EDITOR_FOLDING_ENABLED.equals(property)) {
	...
	protected String[] collectContextMenuPreferencePages() {
		...
		int nIds = 0;
		prefPageIds[nIds++] = "org.icemoon.cdt.ninja.ui.preferences.NinjaFileEditorPreferencePage"; //$NON-NLS-1$
	...
	protected void configureSourceViewerDecorationSupport(SourceViewerDecorationSupport support) {
		...
		support.setMatchingCharacterPainterPreferenceKeys(NinjaFileEditorPreferenceConstants.EDITOR_MATCHING_BRACKETS,
				ColorManager.NINJA_MATCHING_BRACKETS_COLOR);
	}
