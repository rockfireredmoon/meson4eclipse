 ...
 */
public final class NinjaToggleCommentAction extends TextEditorAction {
	 ...
	 */
	public NinjaToggleCommentAction(ResourceBundle bundle, String prefix, ITextEditor editor) {
	...
	public void run() {
		...
		if (!fOperationTarget.canDoOperation(operationCode)) {
			if (shell != null) {
				MessageDialog.openError(shell, NinjaFileEditorMessages.ToggleComment_error_title,
						NinjaFileEditorMessages.ToggleComment_error_message);
			}
			return;
		}
	...
	private boolean isSelectionCommented(ISelection selection) {
		...
		try {
			...
			// For each partition in the text selection, figure out the startline and
			// endline.
			...
			NinjaUIPlugin.log(e); // Should not happen
		}
	 ...
	 *
	 * Note, the implementation has to match
	 * {@link TextViewer}.getTextBlockFromSelection().
	 * 
	 * @param selection
	 ...
	 * @param document
	 *            The document
	 * @return the first line whose start index is in the given range, -1 if there
	 *         is no such line
	 */
	 ...
	 * @param document
	 *            The document
	 * @return <code>true</code> iff each line from <code>startLine</code> to and
	 *         including <code>endLine</code> is prepended by one of the
	...
	/**
	 * For the different content types, get its default comment prefix and store the
	 * prefixes.
	 * 
	 * @param sourceViewer
