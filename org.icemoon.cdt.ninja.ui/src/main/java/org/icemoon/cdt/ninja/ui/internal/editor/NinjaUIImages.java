 ...
 * <p>
 * This class provides {@link Image} and {@link ImageDescriptor} for each named
 * image in the interface. All {@code Image} objects provided by this class are
 * managed by this class and must never be disposed by other clients.
 * </p>
 * <p>
 * For common CDT images see {@link CDTSharedImages}.
 * <p>
 * For common platform images see {@link org.eclipse.ui.ISharedImages}
 * ({@code org.eclipse.ui.PlatformUI.getWorkbench().getSharedImages()}) <br>
...
public class NinjaUIImages {
	...
	public static final String IMG_ETOOL_INCLUDE = "icons/etool16/ninjafile.gif"; //$NON-NLS-1$
	public static final String IMG_ETOOL_SUBNINJA = "icons/etool16/subninja.gif"; //$NON-NLS-1$
	...
	private static SharedImagesFactory imagesFactory = new SharedImagesFactory(NinjaUIPlugin.getDefault());

	/**
	 * Internal method. It lets register image URL from a bundle directly to the
	 * map. It is user responsibility to ensure that a valid URL is passed.
	 *
	 * @param url
	 *            - URL of the image pointing to its location in a bundle (bundle
	 *            entry).
	 *
	 * @noreference This is internal method which is not intended to be referenced
	 *              by clients.
	 */
	...
	/**
	 * The method retrieves an image from the internal repository according to the
	 * given key. The image is managed by image registry and the caller must not
	 * dispose it.
	 *
	 * @param key
	 *            - one of {@code MakeUISharedImages.IMG_} constants.
	 *            <p>
	 *            Reserved for internal usage: the key could be a string
	 *            representation of URL pointing to location of the image in the
	 *            bundle. Such URL key must be registered first with
	 *            {@code register(URL url)}.
	 *            </p>
	 * @return the image from the repository or the default image for missing image
	 *         descriptor.
	 */
	...
	/**
	 * The method retrieves an image descriptor from the internal repository
	 * according to the given key. See also {@link #getImage(String)}.
	 *
	 * @param key
	 *            - one of {@code MakeUISharedImages.IMG_} constants.
	 * @return the image from the repository or
	...
	/**
	 * Retrieves an overlaid image from the internal repository of images. If there
	 * is no image one will be created.
	 *
	 * The decoration overlay for the base image will use the array of provided
	 * overlays. The indices of the array correspond to the values of the 5 overlay
	 * constants defined on {@link IDecoration}, i.e. {@link IDecoration#TOP_LEFT},
	 ...
	 *            the base image key.
	 * @param overlayKeys
	 *            the keys for the overlay images. Must be String[5], i.e. string
	 *            array of 5 elements. Put {@code null} as an element to the array
	 *            if no overlay should be added in given quadrant.
	 */
	...
	/**
	 * Retrieves an overlaid image descriptor from the repository of images. If
	 * there is no image one will be created.
	 *
	 * @param baseKey
	 *            - key of the base image. Expected to be in repository.
	 * @param overlayKey
	 *            - key of overlay image. Expected to be in repository as well.
	 * @param quadrant
	 *            - location of overlay, one of those: {@link IDecoration#TOP_LEFT},
	 *            {@link IDecoration#TOP_RIGHT}, {@link IDecoration#BOTTOM_LEFT},
