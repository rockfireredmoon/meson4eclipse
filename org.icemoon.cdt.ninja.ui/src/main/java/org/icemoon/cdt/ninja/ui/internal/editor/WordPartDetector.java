...
import org.icemoon.cdt.ninja.core.NinjaFileUtil;
...
public class WordPartDetector {
			...
			for (int index = offset - 1; index > 0; index--) {
				...
				if (!Character.isJavaIdentifierPart(c) && c != '-' && c != ' ') {
					...
					if (isFunction) {
						...
						if (builtinFun.equals(NinjaFileConstants.FUNCTION_CALL)) {
							type = WORDPART_TYPE.FUNCTION_CALL;
							int nameOffset = index + 2 + NinjaFileConstants.FUNCTION_CALL.length();
							int endIndex = offset;
	 ...
	 * 
	 * @return {@code true} if the cursor is located in macro, {@code false}
	 *         otherwise.
	 */
	 ...
	 * 
	 * @return {@code true} if the cursor is located in function call, {@code false}
	 *         otherwise.
	 */
	 ...
	 * 
	 * @return {@code true} if the cursor is located on include line, {@code false}
	 *         otherwise.
	 */
	...
	/**
	 * Find word located in the given position. A word is defined here as a sequence
