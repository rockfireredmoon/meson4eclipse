...
public class NinjaPreferencePage extends FieldEditorPreferencePage implements IWorkbenchPreferencePage {
	...
	public NinjaPreferencePage() {
		super(GRID);
		setPreferenceStore(NinjaUIPlugin.getDefault().getPreferenceStore());
	}
	...
	protected void createFieldEditors() {
		...
		BooleanFieldEditor tagetBackgroundEditor = new BooleanFieldEditor(PREF_BUILD_TARGET_IN_BACKGROUND,
				NinjaUIPlugin.getResourceString(TARGET_BUILDS_IN_BACKGROUND), parent);
		...
		RadioGroupFieldEditor edit = new RadioGroupFieldEditor(PREF_BUILD_LAST_TARGET,
				NinjaUIPlugin.getResourceString(BUILD_LAST_TARGET), 1,
				new String[][] { { NinjaUIPlugin.getResourceString(BUILD_LAST_PROJECT), PREF_BUILD_LAST_PROJECT },
						{ NinjaUIPlugin.getResourceString(BUILD_LAST_RESOURCE), PREF_BUILD_LAST_RESOURCE },
						{ NinjaUIPlugin.getResourceString(BUILD_LAST_PROJECTROOT), PREF_BUILD_LAST_PROJECTROOT } },
				parent, true);
		addField(edit);
	}

	public static boolean isBuildTargetInBackground() {
		return NinjaUIPlugin.getDefault().getPreferenceStore().getBoolean(PREF_BUILD_TARGET_IN_BACKGROUND);
	}

	public static void setBuildTargetInBackground(boolean enable) {
		NinjaUIPlugin.getDefault().getPreferenceStore().setValue(PREF_BUILD_TARGET_IN_BACKGROUND, enable);
	}
	...
	public static boolean useProjectRootForLastMakeTarget() {
		return NinjaUIPlugin.getDefault().getPreferenceStore().getString(PREF_BUILD_LAST_TARGET)
