...
class ParserDetection {
	private static final ILog log = MesonPlugin.getDefault().getLog();
	...
	/**
	 * same as {@link #parserDetectors}, but for windows where meson places a
	 * backslash in the command path. Unused (<code>null</code>), when running under
	 * *nix.
	 */
	...
	static {
		...
		final IToolArgumentParser[] posix_cc_args = { 
				...
				new ToolArgumentParsers.LibraryPath_C_POSIX()};
		...
		for (ParserDetector pd : parserDetectors) {
			if (pd instanceof ParserDetectorExt) {
				...
				parserDetectorsWin32
						.add(new ParserDetectorExt(pde.basenameRegex, true, pde.extensionRegex, pde.getParser()));
			} else {
	...
	public static ParserDetectionResult determineDetector(String line, String versionSuffixRegex,
		...
		if (result == null && tryWindowsDectors) {
			...
			if (result == null) {
				...
				if (result == null) {
					// try for meson from mingw suite with short file names under windows
	...
	private static ParserDetectionResult determineDetector(String commandLine, List<ParserDetector> detectors,
		...
		for (ParserDetector pd : detectors) {
			if ((remaining = pd.basenameMatches(commandLine)) != null) {
				return new ParserDetectionResult(
						new DetectorWithMethod(pd, DetectorWithMethod.DetectionMethod.BASENAME), remaining);
			}
		}
		if (versionSuffixRegex != null) {
			...
			for (ParserDetector pd : detectors) {
				if ((remaining = pd.basenameWithVersionMatches(commandLine, versionSuffixRegex)) != null) {
					return new ParserDetectionResult(
							new DetectorWithMethod(pd, DetectorWithMethod.DetectionMethod.WITH_VERSION), remaining);
				}
			}
		}
		// try with extension
		for (ParserDetector pd : detectors) {
			if (pd instanceof ParserDetectorExt
				...
				return new ParserDetectionResult(
						new DetectorWithMethod(pd, DetectorWithMethod.DetectionMethod.WITH_EXTENSION), remaining);
			}
		}
		if (versionSuffixRegex != null) {
			...
			for (ParserDetector pd : detectors) {
				if (pd instanceof ParserDetectorExt && (remaining = ((ParserDetectorExt) pd)
					...
					return new ParserDetectionResult(
							new DetectorWithMethod(pd, DetectorWithMethod.DetectionMethod.WITH_VERSION_EXTENSION),
							remaining);
				}
			}
		}
	 ...
	 * workaround for a
	 * <a href="https://gitlab.kitware.com/meson/meson/issues/16138">bug in Meson
	 * under windows</a>.<br>
	 * See <a href="https://github.com/15knots/meson4eclipse/issues/31">issue #31
	 */
	private static String expandShortFileName(String commandLine) {
		...
		try {
			...
			log.log(new Status(IStatus.ERROR, MesonPlugin.PLUGIN_ID, "CompileCommandsJsonParser#expandShortFileName()",
					e));
		}
		return null;
	}
	...
	static class ParserDetector {
		...
		/**
		 * whether to match a Linux path (which has a forward slash) or a Windows path
		 * with backSlashes
		 */
		 ...
		 *         remaining command-line string (without the portion that matched) is
		 *         returned.
		 */
	 ...
	 * properly, if tool versions are allowed and the tool name contains a filename
	 * extension.
	 *
	 * @author weber
	 */
	...
	static class ParserDetectionResult {
		...
		/**
		 * the remaining arguments of the command-line, after the matcher has matched
		 * the tool name
		 */
